...
sign_exe() {
  #Sign Mesh binaries
  [ "${SIGN}" == "sign" ] && cmd \""/c signtool sign /debug /v /tr http://timestamp.sectigo.com /td sha256 /fd sha256 /a RiVPN/mesh.exe RiV-mesh/meshctl.exe"\"
}
...
build_msi(){
  ...
  light $LIGHTFLAGS -ext WixUIExtension -ext WixUtilExtension.dll -out ${PKGNAME}-${PKGVERSION}-${PKGARCH}-win7-ie.msi ${PKGNAME}-${PKGVERSION}-${PKGARCH}.wixobj
}
